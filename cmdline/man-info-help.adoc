...
All the following commans accept the numeric (universal) argument.

- `C-x`: hold `CTRL` and press `x`
- `M-x`: hold `META` (a.k.a `ALT`) and press `x`
- `C-n`: next line
- `C-p`: prev line
- `C-a`: beginning of line (“a” is the first letter of the alphabet)
- `C-e`: end of line
- `C-f`: forward char
- `C-b`: backward char
- `M-f`: forward word
- `M-b`: backward word
- `M-<` or `b`: beginning of node
- `M\->` or `e`: end of node
- `M-r` move to window line

Run `info info 'scrolling commands'.
