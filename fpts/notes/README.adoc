...
// how many people will help pay it. Partially apply the more generic
...
But then suppose each person paying the price will also give a 1-dollar tip to the person delivering the pizza, that means we would get the result of `divPriceBy` and increment by 1.
...
To get started with handling division by zero (which could extend to other erroneous conditions we'd want to signal) et's update `div` to return `null`
