...
The return type reads a little more FPish, but we are still just returning `null` if the divisor is 0, and will have the same problems as before if `compose2(inc, divPriceBy)` to calculate the price for each person plus the 1-dollar tip.

== Data Constructors Some and None
