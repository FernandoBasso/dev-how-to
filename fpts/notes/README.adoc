...
This approach would be conform to the _tail call_ recursion.
Except ECMAScript (at least as of 2024) does not support tail call optimization so even though the syntax of the next example _looks_ like tail call, it doesn't actually result in any performance benefits and the call stack keeps growing as if not tail call style had been used.
