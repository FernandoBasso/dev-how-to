...
`each` is an iterator method that accepts a code block as argument. For each element of the array, the block is run on that element.
...
It is possible to use the both `lambda` literal or the “stabby" `->` to create a lambda functions.
