...
Notice our `g()` takes a takes some type `T`, and returns some type `U` (`T` and `U` can be the same type, but they _don't have to_).
...
And our examples from erlier still work just the same.
Neat!
