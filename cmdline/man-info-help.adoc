...
If you don't know what a man page name would be, we can search the man page names and their sort descriptions by using `-k`, which accepts a regular expression. `man -k pattern` is the same as `apropos pattern`.

TIP: If `apropos` or `man -k` say “nothing appropriate”, read `man mandb` and then run `mandb` as root.
